...
class LL1Parser:
    def __init__(self, parsing_table_file, start_symbol='S'):
        ...
        # Nuevas propiedades para gestión de ámbitos
        self.symbol_table = SymbolTable()
        self.current_scope = 'global'      # 'global' o 'function'
        self.current_function = None       # Nombre de la función actual
        # Flags para seguimiento de contexto
        self.expecting_declaration_id = False
    ...
    def load_parsing_table(self, file, debug=False):
        ...
        with open(file, newline='', encoding='utf-8') as csvfile: #leer csv
            ...
            for line in lines[1:]:
                ...
                if not parts:#ELIMINACIÖN DE LINEAS VACÍAS
                    continue
    ...
    def parse(self, tokens):
        ...
        while stack and index < len(tokens_copy):#mientras tengamos simbolos por analizar y tokens por consumir el while sigue
            ...
            # =============================================================
            # Detección de contexto para tabla de símbolos (ANTES de procesar)
            # =============================================================
            
            # Detectar declaraciones de variables
            if token_type == '#':
                self.expecting_declaration_id = True
                
            # Detectar nombres de variables en declaraciones
            elif token_type == 'id' and self.expecting_declaration_id:
                if self.current_scope == 'global':
                    self.symbol_table.add_global_var(token_value, 'auto', line, column)
                else:
            ...
            elif token_type in ['funk_not_ret', 'funk_ret']:
                self.expecting_function_name = True
                self.last_function_type = token_type
                
            # Detectar nombres de funciones
            elif token_type == 'id' and self.expecting_function_name:
                
                return_type = 'no_return' if self.last_function_type == 'funk_not_ret' else 'return'
                self.symbol_table.add_function(
                    name=token_value,
                    params=[],  # Inicialmente vacío (se llenará con parámetros)
            ...
            # Detectar fin de funciones
            elif token_type == '}' and self.current_scope == 'function':
                self.current_scope = 'global'
                self.current_function = None
                
            # =============================================================
            # Procesamiento normal del parser
            # =============================================================
